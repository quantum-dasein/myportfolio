...
export const serviceContent: Record<ServiceKey, ServiceContent> = {
  ...
  websites: {
    ...
    proof: [
      { value: "~1 week", label: c("from finished content to live site", "von fertigem Inhalt bis zur Live-Seite") },
      { value: "1.0s", label: c("largest contentful paint, measured on this site", "Largest Contentful Paint, auf dieser Website gemessen") },
      { value: "0", label: c("plugins, licences or monthly builder fees", "Plugins, Lizenzen oder monatliche Baukastengebühren") },
    ],
    faq: [
...
export const insightContent: Record<InsightKey, InsightContent> = {
  ...
  "fidic-platform": {
    ...
    sections: [
      { title: c("Content as product architecture", "Content als Produktarchitektur"), text: c("Contract books, clauses, glossary terms, certification material and expert articles each need their own user journey while sharing one knowledge model. The data layer is typed TypeScript rather than a CMS: every clause, term and quiz question holds all three language variants in its own structure, so a missing translation fails the build instead of shipping a half-empty page. 117 articles, 28 dissected clauses and 25 glossary terms sit on that model.", "Vertragsbücher, Klauseln, Glossarbegriffe, Zertifizierungsmaterial und Fachartikel brauchen je eigene User Journeys bei einem gemeinsamen Wissensmodell. Die Datenebene ist typisiertes TypeScript statt eines CMS: Jede Klausel, jeder Begriff und jede Testfrage hält alle drei Sprachvarianten in der eigenen Struktur — eine fehlende Übersetzung bricht den Build, statt eine halbleere Seite auszuliefern. 117 Artikel, 28 aufgeschlüsselte Klauseln und 25 Glossarbegriffe liegen auf diesem Modell.") },
      ...
      { title: c("A data layer that maintains itself", "Eine Datenebene, die sich selbst pflegt"), text: c("A daily job merges World Bank procurement notices into a cumulative archive of 2,981 records across eight countries, back to 2019. The upstream API only ever returns a rolling window, so a notice that drops out of the feed is gone for good — which is why the pipeline merges rather than overwrites, and exits non-zero on an upstream failure instead of quietly saving an empty snapshot. Each bot commit carries the current open-versus-total count, so the archive's health is readable from the git log alone.", "Ein täglicher Job führt Ausschreibungen der Weltbank zu einem kumulativen Archiv von 2.981 Einträgen aus acht Ländern zusammen, zurück bis 2019. Die Quell-API liefert immer nur ein gleitendes Fenster — eine herausgefallene Ausschreibung ist endgültig weg. Deshalb führt die Pipeline zusammen statt zu überschreiben und endet bei einem Ausfall mit Fehlercode, statt still einen leeren Snapshot zu speichern. Jeder Bot-Commit trägt den Stand offener zu gesamter Ausschreibungen — der Zustand des Archivs ist allein aus dem Git-Log ablesbar.") },
  ...
  "ai-development": {
    ...
    sections: [
      ...
      { title: c("What a client is actually buying", "Was ein Kunde tatsächlich kauft"), text: c("Not production capacity — that is now cheap and widely available. What is scarce is someone who knows the subject well enough to catch the answer that is confidently wrong, who decides what not to build, and who is still there in six months when something quietly breaks. A generated site is a snapshot. A maintained one is an operation, and the difference shows up months after launch.", "Nicht Produktionskapazität — die ist inzwischen billig und breit verfügbar. Knapp ist jemand, der das Thema gut genug kennt, um die selbstbewusst falsche Antwort zu erkennen, der entscheidet, was nicht gebaut wird, und der in sechs Monaten noch da ist, wenn etwas leise kaputtgeht. Eine generierte Website ist eine Momentaufnahme, eine gepflegte ist ein Betrieb — der Unterschied zeigt sich Monate nach dem Launch.") },
    ],
    relatedService: "digital-marketing",
  },
