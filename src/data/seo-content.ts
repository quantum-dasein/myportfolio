...
export const insightContent: Record<InsightKey, InsightContent> = {
  ...
  "ai-development": {
    ...
    sections: [
      ...
      { title: c("Where it is the wrong tool", "Wo sie das falsche Werkzeug ist"), text: c("Anywhere the answer has to be checkably true. A generated paragraph about a FIDIC clause reads exactly as confident when it is wrong, so on subject-matter work the model drafts and the domain knowledge decides. It is also the wrong tool for scope: deciding what not to build is the highest-leverage call on a project and the one least visible in any output.", "Überall dort, wo die Antwort nachprüfbar stimmen muss. Ein generierter Absatz über eine FIDIC-Klausel klingt genauso überzeugt, wenn er falsch ist — bei Fachinhalten liefert das Modell den Entwurf, entschieden wird mit Fachwissen. Ebenso falsch ist sie beim Scope: zu entscheiden, was nicht gebaut wird, ist die wirksamste Entscheidung im Projekt und die in keinem Output sichtbare.") },
      { title: c("Evidence over mythology", "Belege statt Mythologie"), text: c("Build logs, public repositories, analytics context and live URLs make the workflow auditable. The engineering log goes further and records the changes that were measured and then thrown away — a codec that lost on one file in three, an import that added 42 KB for every visitor. Tooling does not produce that record; being accountable for the result does.", "Build-Logs, öffentliche Repositories, Analytics-Kontext und Live-URLs machen den Workflow nachvollziehbar. Das Engineering-Log geht weiter und hält die Änderungen fest, die gemessen und dann verworfen wurden — ein Codec, der bei einer von drei Dateien verlor, ein Import, der jedem Besucher 42 KB aufschlug. Dieses Protokoll erzeugt kein Werkzeug, sondern die Verantwortung für das Ergebnis.") },
    ],
    relatedService: "digital-marketing",
  },
};
